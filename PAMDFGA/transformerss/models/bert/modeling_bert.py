...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_layer = torch.cat([past_key_value[0], key_layer], dim=2) # [8 12 seq+pre 64]
            value_layer = torch.cat([past_key_value[1], value_layer], dim=2) # [8 12 seq+pre 64]
        else:
            key_layer = self.transpose_for_scores(self.key(hidden_states))
            value_layer = self.transpose_for_scores(self.value(hidden_states))

        query_layer = self.transpose_for_scores(mixed_query_layer) # [8 12 seq 64]
        ...
        if attention_mask is not None:
            ...
            attention_scores_g = attention_scores + attention_mask #[8 12 seq seq_pre] + [8,1,1,seq+pre]
        ...
        attention_probs = self.dropout(attention_probs_g)

        # Mask heads if we want to
        if head_mask is not None:
            attention_probs_g = attention_probs * head_mask

        context_layer = torch.matmul(attention_probs_g, value_layer)

        #######################
        if past_key_value !=  None:
            past_key_value_lenn = past_key_value.shape[-2]
        if dep_att_mask != None and past_key_value != None:
            dep_att_mask_temp = torch.zeros(dep_att_mask.shape[0], 1, dep_att_mask.shape[2], past_key_value_lenn).cuda()
            dep_att_mask_new = torch.cat((dep_att_mask_temp,dep_att_mask), 3)
        if (dep_att_mask is not None) and (selector_outputs is not None):
            attention_scores_l = attention_scores + dep_att_mask_new  # [8, 12, 61, 72] + [8,1,61,61] ->[8 1 61 72]
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        self_attn_past_key_value = past_key_value[:2] if past_key_value is not None else None # [2 8 12 11 64]
...
class AttentionSelector(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.selection_mode == 'hard':  # try
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if dep_att_mask is not None:
            ...
            extended_dep_att_mask = (1.0 - extended_dep_att_mask) * -10000.0 # [8, 1, seq, seq]
        else:
