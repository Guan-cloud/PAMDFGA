...
class Trainer(object):
    ...
    def train(self, epoch, data_loader):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            outputs, ehr_attentions1, ehr_attentions2 = model(bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length)
            # outputs = model(bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length)
            grid_mask2d = grid_mask2d.clone()
            loss = self.criterion(outputs[grid_mask2d], grid_labels[grid_mask2d])

            ad_loss1 = alignment.attention_diversity((ehr_attentions1))
            ad_loss2 = alignment.attention_diversity((ehr_attentions2))
            ad_loss  = (ad_loss1 + ad_loss2) / 2
            loss = loss + 0.001 * ad_loss  # ad_loss
            loss.backward()
    ...
    def eval(self, epoch, data_loader, is_test=False):
        ...
        with torch.no_grad():
            for i, data_batch in enumerate(data_loader):
                ...
                outputs, _, _= model(bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length)
                # outputs = model(bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length)
