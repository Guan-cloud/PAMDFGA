...
def collate_fn(data):
    ...
    mask2d_dep = torch.zeros((batch_size,max_pie,max_pie))
    grid_mask2d = fill(grid_mask2d, mask2d_mat)
    sub_mat = torch.zeros((batch_size, max_tok, max_pie), dtype=torch.bool)
    sub_mat_loss_mask = torch.zeros((batch_size, max_pie), dtype=torch.bool)
...
class RelationDataset(Dataset):
    def __init__(self, bert_inputs, grid_labels, grid_mask2d, pieces2word, dist_inputs, sent_length, entity_text,):
        ...
        # self.datasets_dep = datasets_dep
        # self.dep_att_mask = dep_att_mask
...
def process_bert(data, tokenizer, vocab, datasets_dep=None):
    ...
    for index, instance in enumerate(data):
        ...
        _bert_inputs = tokenizer.convert_tokens_to_ids(pieces)  # [28]
        _bert_inputs = np.array([tokenizer.cls_token_id] + _bert_inputs + [tokenizer.sep_token_id])

        length = len(instance['sentence'])
        _grid_labels = np.zeros((length, length), dtype=np.int) # [16 16]
        _pieces2word = np.zeros((length, len(_bert_inputs)), dtype=np.bool) # [16 30]
        _dist_inputs = np.zeros((length, length), dtype=np.int)  # [16 16]
        _grid_mask2d = np.ones((length, length), dtype=np.bool)  # [16 16]
        ...
        for entity in instance["ner"]:
            ...
            _grid_labels[index[-1], index[0]] = vocab.label_to_id(entity["type"])  # 找到词语的NNW二维度位置
...
def load_data_bert(config):
    ...
    # with open('./data/{}/ncbi_test/train.json'.format(config.dataset), 'r', encoding='utf-8') as f:
    #     train_data = json.load(f)
    # with open('./data/{}/ncbi_test/dev.json'.format(config.dataset), 'r', encoding='utf-8') as f:
